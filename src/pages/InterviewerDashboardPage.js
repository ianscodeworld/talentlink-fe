    ...
    return (
        <div>
            ...
            <List
                ...
                renderItem={(demand) => (
                    <List.Item>
                        <Link to={`/hm/demands/${demand.id}`}>
                            <Card hoverable title={demand.jobTitle}>
                                <Paragraph><strong>Status:</strong> <Tag>{demand.status}</Tag></Paragraph>
                                <Paragraph><strong>Created By:</strong> {demand.createdByName}</Paragraph>
                                {/* Specialties 字段可能需要添加到 DemandResponse 中 */}
                                {/* <Paragraph><strong>Specialties:</strong> {demand.specialties?.join(', ')}</Paragraph> */}
                            </Card>
                        </Link>
                    </List.Item>
                )}
